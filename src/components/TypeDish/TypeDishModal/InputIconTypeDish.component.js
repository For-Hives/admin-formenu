...
export function InputIconTypeDishComponent({
	control,
	errors,
	name,
	value,
	...
	const handleFileChange = async event => {
		...
		try {
			...
			if (res.ok) {
				...
				setUploadedImage(uploadedFile) // Update the state with the uploaded image
			}
		} catch (e) {
			console.error(e)
		} finally {
		...
		<div className={'flex flex-col gap-4'}>
			<div className={'flex flex-col gap-2'}>
				<h2 className={'font-kanit text-lg font-medium'}>
					Icône du type de plat
				</h2>
				<p className={'text-sm italic'}>
					Vous pouvez ajouter une icône pour représenter ce type de plat.
				</p>
			</div>
			<div className="group relative flex w-full items-center justify-center rounded-md border border-cyan-900/50 bg-cyan-900/10 p-4">
				{uploadedImage?.url && (
					<Image
						...
						fill={true}
						alt={uploadedImage.alternativeText ?? 'icône'}
						className={
